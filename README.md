...
| **查不到**：出了事不知道谁在什么时候动过 | 没有记录 | 创建、读取、修改都写进 `.skill-manager/operations.json`，失败原因和多目录创建的回滚结果一并记录 |
...
| 扫描与列表 | 扫描登记目录的直接子目录，展示名称、描述、路径、修改时间 |
| 检索 | 名称/描述关键词搜索、来源目录筛选、修改时间升序或降序 |
| 查看详情 | 读取 `SKILL.md` 全文，返回内容指纹用于冲突判断 |
| 编辑保存 | 内置 Markdown 编辑器；中文简介回写 front matter；外部修改冲突检测与强制覆盖 |
